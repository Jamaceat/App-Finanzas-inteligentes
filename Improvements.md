...
## Comportamiento de burbujas flotantes: catálogo de características (asignar-gastos vs. PocketWidget)

**Archivos**: `src/components/floating-expense-point.tsx`, `src/components/pocket-widget.tsx`, `src/app/asignar-gastos.tsx`, `src/hooks/use-bubble-front-order.ts`

`FloatingExpensePoint` (burbujas de gasto en "Asignar gastos") es la implementación más completa de "burbuja flotante arrastrable" de la app. Cuando se lleve alguna de estas características a otro componente de burbujas (como `FloatingBubble` en `PocketWidget`), usar esta lista como checklist de qué se copió y qué falta:

1. **Wander (deambular)**: oscilación continua en X/Y vía `withRepeat(withSequence(withTiming...))` con amplitud y duración con jitter aleatorio ([floating-expense-point.tsx:150-169](src/components/floating-expense-point.tsx#L150-L169)). ✅ portado a `FloatingBubble`.
2. **Arrastre libre (free-drag)**: `Gesture.Pan()` mueve la burbuja siguiendo el dedo cuando no está enfocada, pausando el wander mientras se arrastra y reanudándolo al soltar. ✅ portado a `FloatingBubble` (ver [pocket-widget.tsx:508-522](src/components/pocket-widget.tsx#L508-L522)).
3. **Bring-to-front persistente**: la burbuja tocada/arrastrada sube de `zIndex` y se queda arriba aunque se suelte, hasta que otra burbuja sea la interactuada. Extraído al hook compartido `useBubbleFrontOrder` (`src/hooks/use-bubble-front-order.ts`) y usado por ambos componentes. ✅ portado.
4. **Clamping a los límites de pantalla**: en `floating-expense-point.tsx` todo movimiento (foco, imán hacia tanques, y arrastre libre) se recorta con `Math.max(POINT_WIDTH/2, Math.min(screenWidth - POINT_WIDTH/2, x))` (mismo patrón en Y con `POINT_HEIGHT`), tanto en `onUpdate` como en el valor final que se persiste en `onEnd` ([floating-expense-point.tsx:277-278](src/components/floating-expense-point.tsx#L277-L278), [:288-289](src/components/floating-expense-point.tsx#L288-L289), [:352-353](src/components/floating-expense-point.tsx#L352-L353), [:376-377](src/components/floating-expense-point.tsx#L376-L377)). Así la burbuja nunca puede arrastrarse fuera del viewport. **❌ pendiente en `FloatingBubble`** (`pocket-widget.tsx`): el drag ahí es un offset libre sin ningún `Math.min`/`Math.max` contra `screenWidth`/`screenHeight` (ni contra los límites del `bubbleField`), así que hoy se puede arrastrar una burbuja de tanque fuera de la pantalla visible.
5. **Foco/centrado + imán hacia tanques + tarjeta de detalle**: exclusivo de `FloatingExpensePoint` (flujo de asignar un gasto a un tanque radial); no aplica al listado simple de `FloatingBubble`.
6. **Persistencia de posición entre renders**: `asignar-gastos.tsx` guarda la posición final del drag libre en `customPositions` (estado del padre) vía `onPositionChange`, así que sobrevive a re-renders de la pantalla. `PocketWidget` no persiste la posición del drag en el padre — sí sobrevive mientras el componente no se desmonte, pero se pierde si el widget se cierra y se vuelve a abrir (decisión tomada explícitamente al portar el drag, no un bug).

**Pendiente de implementar**: agregar clamping a los límites de pantalla (punto 4) en `FloatingBubble` de `pocket-widget.tsx`, siguiendo el mismo patrón de `Math.max`/`Math.min` que `floating-expense-point.tsx`, para que las burbujas de tanque tampoco puedan salir de la vista visible al arrastrarlas.
