...
export default function RecurringExpenseListScreen() {
  ...
  const { data: ruleCountRows } = useLiveQuery(
    countActiveRecurringRules({ kind: 'expense', search: searchText }),
    [searchText],
  );
      ...
      <SafeAreaView style={styles.safeArea} edges={['top']}>
        ...
        >
          ...
          <ThemedView style={styles.list}>
            {rules.length === 0 && (
              <ThemedText themeColor="textSecondary">Sin gastos recurrentes todavía.</ThemedText>
            )}
            {rules.map((rule) => (
              <RuleRow
                key={rule.id}
